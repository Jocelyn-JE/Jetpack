...
namespace Jetpack::Client {
	void Program::_connnectToSocket(const char *ip, unsigned int port) {
		...
		if (connect(this->_socketFd, reinterpret_cast<struct sockaddr *>(&data), sizeof(data)) == -1) {
			perror("connect()");
			close(this->_socketFd);
		}
	}

	void Program::_handleMessageFromServer(std::string msg)
	{
		if (msg.starts_with("POS") && msg.ends_with('\n')) {
			...
			if (data.size() != 4)
				return;
			this->_graphic.setPosRectangle(atoi(data[1].c_str()), sf::Vector2f{static_cast<float>(atof(data[2].c_str())), static_cast<float>(atof(data[3].c_str()))});
		}
		if (msg.starts_with("INIT") && msg.ends_with('\n')) {
			...
			if (data.size() != 3)
				return;
			this->_graphic.addNewPlayer(atoi(data[1].c_str()), atoi(data[2].c_str()));
		}
		if (msg.starts_with("RESET") && msg.ends_with('\n')) {
			...
			if (data.size() != 2)
				return;
			this->_graphic.setPosRectangle(atoi(data[1].c_str()), sf::Vector2f{0.0f, 0.0f});
		}
	}

	void Program::_sniffANetwork() {
		while (this->_isOpen) {
			char buffer[1024] = {0};
			this->_interactionMutex.lock();
			int bytesReceived = recv(this->_socketFd, buffer, sizeof(buffer) - 1, MSG_DONTWAIT);
	...
	void Program::_sendPlayerInput(UserInteractions_s event) {
		this->_interactionMutex.lock();
	...
	Program::Program(const char *ip, unsigned int port, Jetpack::Logger &logger)  : _logger(logger), _graphic(_sendUserInteraction)
	{
