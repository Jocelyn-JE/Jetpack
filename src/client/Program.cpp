...
void jetpack::Client::Program::_setPlayerData(
    ...
    uint32_t yPosInt = (msg[24] << 24) | (msg[25] << 16) | (msg[26] << 8) | msg[27];
    player.y_pos = ntohl(yPosInt);
    uint32_t coinsInt = (msg[28] << 24) | (msg[29] << 16) | (msg[30] << 8) | msg[31];
...
void jetpack::Client::Program::_setCoinData(std::vector<unsigned char> msg) {
    if (msg.size() < sizeof(coinsPos_s)) {
        this->_logger.log("Invalid coin data size: " + std::to_string(msg.size()));
        return;
    }
    coinsPos_s coin;
    ...
    this->_logger.log("Coin Position: X=" + std::to_string(coin.x_pos) + ", Y=" + std::to_string(coin.y_pos));
    this->_logger.log("Coin ID: " + std::to_string(coin.coinId));
    this->_graphic.addNewCoin(coin.coinId);
    this->_graphic.setPosCoin(coin.coinId, sf::Vector2f(coin.x_pos, coin.y_pos));
}

void jetpack::Client::Program::_setLaserData(std::vector<unsigned char> msg) {
    if (msg.size() < sizeof(obstacle_t)) {
        this->_logger.log("Invalid obstacle data size: " + std::to_string(msg.size()));
        return;
    }
    obstacle_t obstacle;
    uint32_t xPosInt = (msg[0] << 24) | (msg[1] << 16) | (msg[2] << 8) | msg[3];
    ...
    int obstacleId = 0;
    if (msg.size() >= 12) {
        uint32_t idInt = (msg[8] << 24) | (msg[9] << 16) | (msg[10] << 8) | msg[11];
        obstacleId = ntohl(idInt);
    }
    this->_logger.log("Obstacle Position: X=" + std::to_string(obstacle.x_pos) + ", Y=" + std::to_string(obstacle.y_pos));
    this->_logger.log("Obstacle ID: " + std::to_string(obstacleId));
    this->_graphic.addNewLaser(obstacleId);
    this->_graphic.setPosLaser(obstacleId, sf::Vector2f(obstacle.x_pos, obstacle.y_pos));
}
