    ...
    if (payload.dataId == PayloadType_t::NAME) {
        if (payload.dataCount > 1)
            throw NetworkException(
                "There is too much payload for this action expected 1 data "
                "currently: " +
        ...
        try {
            ...
            if (selectResult > 0 && FD_ISSET(maxfd, &readfds)) {
                try {
                    ...
                    this->_logger.log("Read disconnected" +
                                      std::string(e.what()));
                    this->_socket.closeSocket();
                    continue;
                }
            ...
            if (!buffer.empty()) {
                try {
                    ...
                    this->_logger.log("Network error in message handling: " +
                                      std::string(e.what()));
                }
            }
        } catch (const Socket::SocketError &e) {
...
void jetpack::Client::Program::_sendChangeUsername() {
    try {
        if (this->_socket.getSocketFd() == -1) {
            std::cerr << "Cannot send username: not connected to server"
                      << std::endl;
            return;
        }
        Header_t header{};
        ...
        this->_socket.writeToSocket<u_int16_t>(valueHeaderBigEndian);
        this->_socket.writeToSocket<u_int16_t>(valuePayloadBigEndian);
