...
namespace jetpack::Client {
	void CoinStats::setCoinsAmount(unsigned int coinsAmount) {
		this->_coinsAmount = coinsAmount;
	}

	void CoinStats::display(sf::RenderWindow &window) {
		this->_coinSprite.display(window);
		window.draw(this->_coinAmountText);
	}

	void CoinStats::compute() {
		auto newXPosition = 1325 - ((std::to_string(this->_coinsAmount).length() - 1) * 20);
	...
	CoinStats::CoinStats() :
		_coinSprite("./src/client/assets/coins_sprite_sheet.png", {193, 170}, 1, 1, {0.35, 0.35})
	{
