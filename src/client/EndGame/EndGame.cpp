...
void jetpack::Client::EndGame::compute(
    int id, const std::map<unsigned int, std::pair<std::unique_ptr<Player>,
    ...
    for (auto &s : listPlayer) {
        if (s.second.first->isDead() == true) idDeadPlayer.push_back(s.first);
    }
    if (!idDeadPlayer.empty()) {
        bool isCurrentPlayerDead =
            std::find(idDeadPlayer.begin(), idDeadPlayer.end(), id) !=
            idDeadPlayer.end();
